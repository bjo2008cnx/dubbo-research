...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
  ...
  /**
   * 使用loadbalance选择invoker.</br> a)先lb选择，如果在selected列表中 或者 不可用且做检验时，进入下一步(重选),否则直接返回</br> b)重选验证规则：selected > available
  ...
  protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected)
    ...
    boolean sticky = invokers.get(0).getUrl()
        .getMethodBooleanParameter(methodName, RpcConstants.CLUSTER_STICKY_KEY, RpcConstants.DEFAULT_CLUSTER_STICKY);
    {
  ...
  private Invoker<T> doselect(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected)
    ...
    if ((selected != null && selected.contains(invoker))
      ...
      try {
        ...
        logger.error(
            "clustor relselect fail reason is :" + t.getMessage() + " if can not slove ,you can set cluster.availablecheck=false in url",
            t);
      }
    }
    return invoker;
