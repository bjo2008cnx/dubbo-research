...
public abstract class Builder<T> implements GenericDataFlags {
  ...
  private static Builder<?> newArrayBuilder(Class<?> c) {
    ...
    cwt.append(cn).append(" v = (").append(cn)
  ...
  private static Builder<?> newObjectBuilder(final Class<?> c) {
    ...
    if (fns != null) {
      ...
      do {
        ...
        for (Field tf : fs) {
          ...
          if (Modifier.isStatic(mod) || Modifier.isFinal(mod) || tf.getName()
              .equals("this$0")) // skip static or inner-class's 'this$0' field.
          {
            continue;
          }
          if (Modifier.isTransient(mod)) {
    ...
    // writeObject code.
    StringBuilder cwf = new StringBuilder("protected void writeObject(Object obj, ").append(GenericObjectOutput.class.getName())
        .append(" out) throws java.io.IOException{");
    cwf.append(cn).append(" v = (").append(cn).append(")$1; ");
    cwf.append("$2.writeInt(fields.length);");

    // readObject code.
    StringBuilder crf = new StringBuilder("protected void readObject(Object ret, ").append(GenericObjectInput.class.getName())
        .append(" in) throws java.io.IOException{");
    crf.append("int fc = $2.readInt();");
    crf.append("if( fc != ").append(fs.length).append(" ) throw new IllegalStateException(\"Deserialize Class [").append(cn)
        .append("], field count not matched. Expect ").append(fs.length).append(" but get \" + fc +\".\");");
    crf.append(cn).append(" ret = (").append(cn).append(")$1;");

    // newInstance code.
    StringBuilder cni = new StringBuilder("protected Object newInstance(").append(GenericObjectInput.class.getName())
        .append(" in){ return ");
  ...
  private static Builder<?> newEnumBuilder(Class<?> c) {
    ...
    StringBuilder cpf = new StringBuilder("public Object parseFrom(").append(GenericObjectInput.class.getName())
        .append(" in) throws java.io.IOException{"); // parseFrom code.
    cpf.append("String name = $1.readUTF(); if( name == null ) return null; return (").append(cn).append(")Enum.valueOf(").append(cn)
        .append(".class, name); }");
