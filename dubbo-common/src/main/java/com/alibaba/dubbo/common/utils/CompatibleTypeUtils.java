...
public class CompatibleTypeUtils {
  ...
  public static Object convertIntegerLikeType(Object value, Class<?> clazz) {
    if (value == null || clazz == null || value.getClass().equals(clazz)) {
      return value;
    }
  ...
  /**
   * 兼容类型转换。null值是OK的。如果不需要转换，则返回原来的值。 进行的兼容类型转换如下：（基本类对应的Wrapper类型不再列出。） <ul> <li> String -> char, enum, Date <li> byte, short, int, long
   * -> byte, short, int, long <li> float, double -> float, double </ul>
   */
