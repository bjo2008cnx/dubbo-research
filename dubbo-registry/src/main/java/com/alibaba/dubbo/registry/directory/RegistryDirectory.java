    ...
    if (urls == null || urls.size() == 0) { // 黑白名单限制
      ...
      for (URL url : urls) {
        if (RpcConstants.ROUTE_PROTOCOL.equals(url.getProtocol())) {
          ...
          logger.error("Unsupported protocol " + url.getProtocol() + " in notified url " + url + " from registry " + getUrl().getAddress()
  ...
  public List<Invoker<T>> doList(Invocation invocation) {
    if (forbidden) {
      throw new RpcException(RpcException.FORBIDDEN_EXCEPTION,
          "Forbid consumer " + NetUtils.getLocalHost() + " access service " + getInterface().getName() + " from registry " + getUrl()
              .getAddress() + " use dubbo version " + Version.getVersion() + ", Please check registry access list (whitelist/blacklist).");
    }
