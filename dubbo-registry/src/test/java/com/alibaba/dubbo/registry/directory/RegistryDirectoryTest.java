...
public class RegistryDirectoryTest {
  ...
  public void testDubbo1UrlWithGenericInvocation() {
    ...
    Assert.assertEquals(1, invokers.size());
    Assert.assertEquals("dubbo://0.0.0.0:9092/com.alibaba.dubbo.demo.DemoService?callbacks=100&check=false&methods=getXXX1,getXXX2,getXXX3",
        invokers.get(0).toString());
  ...
  /**
   * 1. notify twice, the second time notified router rules should completely replace the former one. 2. notify with no router url, do
   * nothing to current routers 3. notify with only one router url, with router=clean, clear all current routers
   */
  @Test
